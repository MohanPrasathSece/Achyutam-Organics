...
const OrderManagement = () => {
    ...
    const exportToCSV = () => {
        ...
        link.setAttribute("download", `Achyutam_Orders_${exportRange}_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    // Helper: safely parse items from order (JSONB array OR JSON string fallback)
    const parseItems = (order: any): any[] => {
        if (!order) return [];
        // New schema: items is a JSONB column (already an array)
        if (Array.isArray(order.items)) return order.items;
        // Old schema fallback: products was a JSON string
        try { return JSON.parse(order.products || "[]"); } catch { return []; }
    };

    // Helper: format shipping address from object or string
    const formatAddress = (order: any): string => {
        ...
        return order.address || order.shipping_address || "";
    };
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-600 leading-relaxed bg-slate-50 p-4 rounded-2xl">{formatAddress(selectedOrder)}</p>
                                    </div>
                                    {selectedOrder.notes && (
                                        <div>
                                ...
                                <div className="space-y-4 md:text-right">
                                    <div>
                                        ...
                                        <div className="space-y-2 bg-slate-50 p-4 rounded-2xl">
                                            {parseItems(selectedOrder).map((item: any, i: number) => (
