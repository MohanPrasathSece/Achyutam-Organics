...
const getDatabaseProducts = async (): Promise<FeaturedProduct[]> => {
  try {
    ...
    return (data || []).map((product: any) => {
      ...
      let imageSrc = ghee250; // default fallback
      if (product.images && product.images.length > 0) {
        ...
        if (imagePath.startsWith('/product-images/')) {
          ...
          if (imagePath.includes('ghee_250gm.jpeg')) imageSrc = ghee250;
          else if (imagePath.includes('ghee_500ml.jpeg')) imageSrc = ghee500;
      ...
      return {
        ...
        deliveryInfo: product.delivery_info || 'Standard Delivery',
        description: product.description || 'Quality product from Achyutam Organics',
        weight: product.weight || 0,
        unit: product.unit || 'gm',
        shelfLife: product.shelf_life || '6 months',
        storage: product.storage || 'Store in cool, dry place',
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        console.error('Using fallback products due to error:', error);
      }
    };
    fetchProducts();
  }, []);
